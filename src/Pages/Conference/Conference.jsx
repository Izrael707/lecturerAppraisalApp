...
useEffect(() =>{
    ...
    const UploadDocument = () => {
        ...
        if(AppraisalUserID == userId){
            ...
            if(comfirmUserID == userId){
                ...
                const imageRef = ref(store, `${reviewID}/Conference/${docUpload1.name + v4()}`)
                uploadBytes(imageRef, docUpload1).then(() =>{
                    addDoc(Appraisal_Process, {userID: userId, documentType: 'Conference', documentName: docUpload1.name, count: 1, reviewID: reviewID })
                    Swal.fire({
                        title: 'Success',
  ...
  return (
    <div className='container'>
        <div className='popup'>
            <div className="form">
                <h2>Upload Conference Document</h2>
            
                <div className='form-group'>
                    <label for="fn" className='text-bold'>Conference</label>
                    <input 
